...
export default function EventActionMenu({
    ...
    // ── Non-orbit mode: full feature set with positioning wrapper ─────────────
    if (!isOrbitMode) {
        ...
        return (
            <div
                ...
                style={{ position: 'fixed', zIndex: 100, left, top, width: menuW }}
    ...
    // ── Orbit mode: stripped layout, owned by TetheredBubble shell ────────────
    return renderOrbitContent()

    // ── Orbit: Preview Window — Title/Time + Edit/Delete + Color Arc ────────────
    function renderOrbitContent() {
        const timeStr = `${toTimeInputValue(event.startH)} - ${toTimeInputValue(event.endH)}`

        return (
            <div className="bubble-orbit-layout">
                {/* Top: Information */}
                    ...
                    <div className="apple-menu-notes-section">
                        ...
                        {isEditingNotes ? (
                            <textarea
                                ...
                                title="Event Notes" aria-label="Event Notes"
                                value={event.notes || ''}
                    ...
                    <div className="apple-menu-colors">
                        {EVENT_COLORS.map((c) => (
                            <button
                                ...
                                title={`Set color to ${c}`}
                                aria-label={`Set color to ${c}`}
                                className={`apple-color-dot ${c === event.color ? 'is-active' : ''}`}
